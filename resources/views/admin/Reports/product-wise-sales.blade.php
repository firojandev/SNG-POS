    ...
    <main class="main-content">
        ...
        <div class="theme-card mb-4">
            ...
            <div class="theme-card-body">
                <form action="{{ route('sales-report.product-wise') }}" method="GET" class="row g-3">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-2">
                        <label class="form-label">&nbsp;</label>
                        <a href="{{ route('sales-report.export-product-wise-csv', ['start_date' => $startDate, 'end_date' => $endDate]) }}" class="btn btn-sm btn-success w-100">
                            <i class="fa fa-download"></i> Export CSV
                        </a>
                    </div>
                    <div class="col-md-2">
                        <label class="form-label">&nbsp;</label>
                        <button type="button" class="btn btn-sm btn-secondary w-100" onclick="window.location.href='{{ route('sales-report.product-wise') }}'">
                    ...
                    <div class="theme-card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <h3 class="mb-0">{{ number_format($summary['total_products_sold']) }}</h3>
                            </div>
                    ...
                    <div class="theme-card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <h3 class="mb-0">{{ number_format($summary['total_quantity_sold']) }}</h3>
                            </div>
                    ...
                    <div class="theme-card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <h3 class="mb-0 text-success">{{ get_option('app_currency') }}{{ number_format($summary['total_sales'], 2) }}</h3>
                            </div>
        ...
        <div class="row mb-4">
            ...
            <div class="col-lg-6 mb-4">
                <div class="theme-card">
                    <div class="theme-card-header">
                        <h6 class="theme-card-title">Category Breakdown</h6>
                    </div>
                    <div class="theme-card-body">
        ...
        <div class="theme-card">
            <div class="theme-card-header d-flex justify-content-between align-items-center">
                <h6 class="theme-card-title">Product-Wise Sales Details</h6>
                <div class="text-muted">
                    <small>Total: {{ number_format($productSales->count()) }} products</small>
                </div>
            </div>
            <div class="theme-card-body">
                <div class="table-responsive">
                        ...
                        <tbody>
                            @forelse($productSales as $index => $sale)
                                <tr>
                                    ...
                                    <td><code>{{ $sale->product->sku ?? 'N/A' }}</code></td>
                                    <td>
                                        <span class="badge bg-secondary">{{ $sale->product->category->name ?? 'Uncategorized' }}</span>
                                    </td>
                                    <td class="text-center">
                                        <span class="badge bg-info">{{ number_format($sale->total_quantity_sold) }}</span>
                                    </td>
                                    <td class="text-end text-success">
                                        <strong>{{ get_option('app_currency') }}{{ number_format($sale->total_sales, 2) }}</strong>
                                    </td>
                                    <td class="text-end">
                                        {{ get_option('app_currency') }}{{ number_format($sale->total_vat, 2) }}
                                    </td>
                                    <td class="text-center">{{ $sale->order_count }}</td>
                                    <td class="text-end">{{ get_option('app_currency') }}{{ number_format($sale->avg_unit_price, 2) }}</td>
                                    <td class="text-center">
                                        @if(($sale->product->stock_quantity ?? 0) > 10)
                                            <span class="badge bg-success">{{ number_format($sale->product->stock_quantity) }}</span>
                                        @elseif(($sale->product->stock_quantity ?? 0) > 0)
                                            <span class="badge bg-warning">{{ number_format($sale->product->stock_quantity) }}</span>
                                        @else
                            ...
                            @empty
                                <tr>
                                    <td colspan="10" class="text-center text-muted">No products sold in this period</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
...
@push('js')
    ...
    <script>
        ...
        /**
         * Product Wise Sales Report Configuration
         */
        window.productWiseSalesConfig = {
            currency: '{{ get_option('app_currency', '$') }}',
            dateFormatPhp: '{{ get_option('date_format', 'Y-m-d') }}'
        };

        // Category data for chart
        window.categoryData = @json($categoryBreakdown);
    </script>

    <!--============== Product Wise Sales Report JS =================-->
    <script type="text/javascript" src="{{asset('admin/partial/js/product-wise-sales.js')}}"></script>
    <!--============== End Product Wise Sales Report JS =================-->
@endpush
